...
\paragraph{a)} To translate our problem into a linear program we start by transforming the given constraint. Let $x$ be a certain number. We want to constrain the absolute value of $x$ to be smaller or equal to a certain number:
\[
    |x|≤\text{value}
\]
...
\paragraph{(b)}
We know convert the problem into standard form:
\begin{align*}
     x₁⁺- x₁⁻ && +x₂⁺- x₂⁻ && +3  &+x₄&   &   &   &   &    & ≤ && 5\\
    -x₁⁺+ x₁⁻ && +x₂⁺- x₂⁻ && +3  &   &+x₅&   &   &   &    & ≤ && 5\\
     x₁⁺- x₁⁻ && -x₂⁺+ x₂⁻ && -3  &   &   &+x₆&   &   &    & ≤ && 5\\
...
\begin{align*}
    readyNuclear_0 &≤ 0.2 · (readyCoal_0 + e_0)\\
                   &…\\
    ∑_{i=0}^{15-1} readyNuclear_{T-i} &≤ 0.2 · ∑_{i=0}^{20-1} readyCoal_{T-i} +
                                         0.2 · e_T
\end{align*}
