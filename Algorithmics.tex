...
\begin{itemize}
    \item $f₁(x₁, x₂) = min\{x₁,x₂\}$ with $0 ≤ xᵢ ≤ C, i = 1,2$.
...
The following solutions for each subproblem are reworded versions of the
solutions suggested in a forums post~\cite{Informatik_Forum_LP_Functions}.

\paragraph{$f₁(x₁, x₂) = min{x_1,x_2}$}

We start with the constraints. We know that the minimum of two numbers minus
either one of the numbers has to be smaller or equal to 0:
\begin{alignat*}{1}
    minimum_{x_1,x_2} - x₁ &≤ 0\\
    minimum_{x_1,x_2} - x₂ &≤ 0
\end{alignat*}

We could now insert a „very small” (= large negative) number for
\[ minimum_{x_1,x_2} \] and it would satisfy both constraints. To get to the
minimum we need to increase this number till it reaches its maximum. Therefore
...
We know that the absolute value of a number like for example $y = x₁ − x₂$
minus or plus the value of the number has to be bigger or equal to 0. We now
need the smallest value which fits these constraints. Therefore we get:
...
constraints is smaller or equal than a certain constant value:
\[
    maximize ~~ xor_{x₁,x₂}
\]
...
\[
    -x≤value
\]
Knowing this we now translate the given constraint:
\[
...
$pᵢ$. The goal is to maximize your profit while not violating the following
constraints:
...
We need to know which investment we should make. Therefore we choose $x_i$ to
be the a binary decision variable which is set to 1 if we choose investment $i$
or 0 otherwise.
