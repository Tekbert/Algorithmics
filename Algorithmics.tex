...
weight of the arc is smaller than the current shortest path $u$. If that is the
case we replace the shortest path to node $u$ with the new value.\\

We now apply this algorithm to the graph shown in
Figure~\ref{figure:Delay_Shortest_Path}. We use node “A” as source:
...
To get the shortest path we just select $δ$ for the sink node and calculate the
way back. For example if we want to get the shortest path to node $F$ with the
delay constraint $9$ we see that the value for the shortest path ($5$) just
changed in the row before. This means we that in the step before we selected a
new edge. This edge has to be $(D,F)$ with weight $1$, since the delay of the
other possible edge is larger than the delay constraint $9$.\\

We now go to the column for the shortest for $D$ and check in which row the
value changed from $4$ to a higher value. This was the case in the row with
delay $7$. This in turn means that we selected edge $(B,D)$ in the step before.
...
delay constrain to a maximum value of $9$.
