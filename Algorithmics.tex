...
\section{Exercise 1 (Randomized Algorithm for 3-Coloring of a Graph)}
...
The pseudo-code above assigns a random color to each edge of a graph. The code
...
\]
\[
    \text{Number of satisfied edges}: Z = ∑_{(i,j)∈E(G)}
\]
\[
    E[Z] =
    ∑_{(i,j)∈E(G)}E[Z_{ij}] =
    ∑_{(i,j)∈E(G)}Pr[e_{ij} \text{ is satisfied}] =
    \frac{6}{9} =
    \frac{2}{3}
\]
